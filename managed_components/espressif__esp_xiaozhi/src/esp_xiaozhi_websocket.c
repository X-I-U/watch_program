...
esp_err_t esp_xiaozhi_websocket_start(esp_xiaozhi_websocket_handle_t handle)
{
    ...
    /* [DIAG] Step1: 打印 WS 配置/鉴权状态(不泄露完整 token), 排查"服务器收到hello后关连接" */
    char hdr_redacted[1024];
    snprintf(hdr_redacted, sizeof(hdr_redacted), "%s", websocket->headers);
    char *auth = strstr(hdr_redacted, "Bearer ");
    char *mask_from = NULL;
    if (auth) {
        mask_from = auth + 7;
    } else {
        auth = strstr(hdr_redacted, "Authorization:");
        if (auth) {
            mask_from = strchr(auth, ' ');
            if (mask_from) mask_from += 1;
        }
    }
    if (mask_from) {
        for (char *p = mask_from; *p && *p != '\r' && *p != '\n'; p++) *p = '*';
    }
    ESP_LOGI(TAG, "[DIAG] WS url=%s", websocket->url);
    ESP_LOGI(TAG, "[DIAG] WS token %s (len=%d) | proto-v=%s",
             (websocket->token[0] != '\0') ? "PRESENT" : "EMPTY",
             (int)strlen(websocket->token), websocket->protocol_version);
    ESP_LOGI(TAG, "[DIAG] WS headers:\n%s", hdr_redacted);
    ...
    esp_websocket_client_config_t websocket_cfg = {
        ...
        .task_stack = 8192, /* 默认4KB 跑 TLS握手+4096位RSA验签会栈溢出(0x4290乱码错误), 加大 */
    };
